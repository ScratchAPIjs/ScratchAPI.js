...
$jsal.postProfileComment=(username,content,option)=>{
  ...
  fetch("/site-api/comments/user/"+username+"/add/",{method:"POST",headers:{'Accept':'text/html, */*; q=0.01','Content-type':'application/x-www-form-urlencoded; charset=UTF-8','X-Requested-With':'XMLHttpRequest','X-CSRFToken':document.cookie.match(/(; |^)scratchcsrftoken=(.+?);/)[2]},body:JSON.stringify(json),credentials:'include'})
}
...
$jsal.readSession=(afterreading)=>{
  fetch("/session/",{headers:{'X-Requested-With':'XMLHttpRequest'},credentials:'include'}).then(a=>a.json()).then(a=>{
    ...
    $jsal.postProjectComment=(projectid,content,option)=>{
      ...
      fetch("https://api.scratch.mit.edu/proxy/comments/project/"+projectid,{method:"POST",headers:{'Accept':'application/json','Content-type':'application/json','x-token':$jsal.session.user.token,'X-CSRFToken':document.cookie.match(/(; |^)scratchcsrftoken=(.+?);/)[2]},body:JSON.stringify(json),credentials:'include'})
    }
    $jsal.postStudioComment=(studioid,content,option)=>{
      ...
      fetch("https://api.scratch.mit.edu/proxy/comments/studio/"+studioid,{method:"POST",headers:{'Accept':'application/json','Content-type':'application/json','x-token':$jsal.session.user.token,'X-CSRFToken':document.cookie.match(/(; |^)scratchcsrftoken=(.+?);/)[2]},body:JSON.stringify(json),credentials:'include'})
    }
