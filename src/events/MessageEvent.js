...
class MessageEvent {
  ...
  async start() {
    this.unreadMessage = await this.client.adapter.get(
      `${Routes.API.user(this.client.username)}messages/count/`
    );
    // console.log(messages)
    this.id = setInterval(
      this.eventUpdate.bind(this),
      Constants.API.MESSAGE_EVENT_FREQUENCE
    );
  }

  async eventUpdate() {
    let unreadMessage = (
      await this.client.adapter.get(
        `${Routes.API.user(
          this.client.username
        )}messages/count/?timestamp=${new Date().getTime()}`
      )
    ).data.count;
    if (unreadMessage > this.unreadMessage) {
      let uncheckedMessage = unreadMessage - this.unreadMessage;
      let xToken = (
        await this.client.adapter.request(`https://${Servers.GENERAL}/session/`)
      ).data.user.token;
      let messages = (
